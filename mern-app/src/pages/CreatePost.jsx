...
const CreatePost = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if( form.prompt && form.photo ) {
      ...
      try {
        const response = await fetch('https://mern-openai-img-app.onrender.com/api/v1/post', {
          method: 'POST',
          headers: {
  ...
  const generateImg = async () => {
    if(form.prompt) {
      try {
        ...
        const response = await fetch('https://mern-openai-img-app.onrender.com/api/v1/dalle', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
      ...
      >
        <div className="flex flex-col gap-5">
          ...
          <FormField 
            ...
            type="text"
            name="prompt"
            placeholder="golf 1 gti from 80s in space and earth on the background"
            value={form.prompt}
            handleChange={handleChange}
            isSurpriseMe
        ...
        <div className="mt-10 ">
          <p className="mt-2 text-[#666e75] text-[14px]">
            Once you have created image you want to share it with others in the community
          </p>
